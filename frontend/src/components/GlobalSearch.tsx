    ...
    <div ref={dropRef} style={{ position: 'relative', flex: 1, maxWidth: 480 }}>
        ...
        {query && (
          <button onClick={() => { setQuery(''); setResults([]); setOpen(false); }}
            style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)',
              background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: 'var(--text-lg)', lineHeight: 1 }}>
            ×
          </button>
        )}
        {!query && (
      ...
      {open && (query.length >= 2) && (
        <div style={{
          ...
          {loading && (
            <div style={{ padding: '14px 16px', fontSize: 'var(--text-base)', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 12, height: 12, border: '2px solid var(--border)', borderTopColor: 'var(--primary)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
              Searching...
            </div>
          )}

          {!loading && results.length === 0 && (
            <div style={{ padding: '14px 16px', fontSize: 'var(--text-base)', color: 'var(--text-muted)' }}>
          ...
          {!loading && results.length > 0 && (
            <>
              ...
              {(['ip','lease','subnet','supernet','scope','dns'] as const).map(type => {
                ...
                return (
                  <div key={type}>
                    <div style={{ padding: '8px 16px 4px', fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', background: 'var(--bg-primary)' }}>
                      {TYPE_LABEL[type]}
                    </div>
                    {group.map((r, i) => {
                      ...
                      return (
                        ...
                        >
                          <span style={{ color: TYPE_COLOR[type], fontSize: 'var(--text-lg)', flexShrink: 0 }}>{TYPE_ICON[type]}</span>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontWeight: 600, fontSize: 'var(--text-base)', fontFamily: 'var(--font-mono)', color: 'var(--text-primary)' }}>{r.title}</div>
                            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.subtitle}</div>
                          </div>
                          {r.status && (
                            <span className={`badge ${r.status === 'Active' ? 'badge-green' : r.status === 'dhcp' ? 'badge-blue' : 'badge-gray'}`} style={{ fontSize: 'var(--text-xs)', flexShrink: 0 }}>
                              {r.status}
                            </span>
                          )}
                          {r.type === 'scope' && r.meta.percent_used !== undefined && (
                            <span style={{ fontSize: 'var(--text-xs)', color: parseFloat(r.meta.percent_used) > 80 ? '#dc2626' : 'var(--text-muted)', fontWeight: 600, flexShrink: 0 }}>
              ...
              <div style={{ padding: '8px 16px', fontSize: 'var(--text-xs)', color: 'var(--text-muted)', background: 'var(--bg-primary)', textAlign: 'center' }}>
