...
const DISMISS_KEY_PREFIX = 'ddivault-update-failure-dismissed-';
...
// Surfaces a failed Update-DDIVault.ps1 run (read from /api/system/last-update-status,
// written by the script's Write-StatusJson) as a dismissible banner. Admin-only —
// unlike UpdateNotifier (update-available, shown to everyone), this exposes internal
// operational detail (stage/error code/error message) that only an admin needs to act on.
export default function UpdateFailureBanner({ onGoToSettings }: { onGoToSettings?: () => void }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        ...
        {info.rolledBack ? (
          <span>
            <strong>An update failed</strong> at {stageLabel} and was automatically rolled back — DDIVault is
            running normally on the previous version, but this needs to be fixed
            {info.errorCode ? ` (error code ${info.errorCode})` : ''}.
          </span>
        ) : (
          <span>
            <strong>An update failed</strong> at {stageLabel} and the automatic rollback also failed — DDIVault may be
        ...
        {onGoToSettings ? (
          ...
          <a href="/?tab=settings&settingsTab=updates" style={{ color: '#fff', textDecoration: 'underline', whiteSpace: 'nowrap' }}>
            View details
          </a>
        )}
      </div>
