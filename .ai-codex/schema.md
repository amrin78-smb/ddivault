...
Also in this file: `update_updated_at()` trigger function (used by `ddi_servers`, `ipam_subnets`, `ipam_supernets`, `ipam_addresses`); a `nocvault_readonly`/`claude_readonly` cross-DB grant DO-block (GRANT SELECT ON ALL TABLES + ALTER DEFAULT PRIVILEGES, no-op if role absent), followed by narrowing REVOKE/GRANT blocks for `app_settings`/`api_keys` (views), `ddi_servers` (column-level grant), and `smtp_config` (view) — see Privilege notes. Three views: `app_settings_public`, `api_keys_public`, `smtp_config_public`.
...
- `ddi_servers` → **column-level `GRANT SELECT (...)`** excluding `ps_username`/`ps_password` (no view — this table has no secret/non-secret mix packed into one generic column the way `app_settings` does, so a plain column allowlist is sufficient).
- `smtp_config` → `smtp_config_public` view: every column except `password`.
- **The 2026-07 pass's original code comment (and this file's own previous text) claimed `smtp_config`/`ddi_servers.ps_password` were "already safe" because no `*_public` view existed for them — this was WRONG.** Not creating a view does nothing to revoke a table's pre-existing blanket `GRANT SELECT ON ALL TABLES`; both `ddi_servers.ps_password` and `smtp_config.password` were live-verified STILL fully readable by `claude_readonly` on 2026-07-23, the whole time the "FIXED" claim stood. Actually fixed now (see `scripts/schema.sql`'s dedicated REVOKE/GRANT blocks for each table, same style/placement as the `app_settings`/`api_keys` block).
- A REVOKE+GRANT (view or column-level) block runs immediately after the blanket grant in `schema.sql`, every time it runs, for all four tables — so none of this can be silently re-opened by a future deploy re-running the blanket grant.
