...
`ddi_servers`  id(PK,SERIAL) | hostname TEXT NOT NULL | ip_address INET | role TEXT DEFAULT 'both' | description TEXT | is_active BOOLEAN | last_polled TIMESTAMPTZ | poll_status TEXT | poll_error TEXT | created_at/updated_at TIMESTAMPTZ (trigger `trg_ddi_servers_updated`) | health_score INT | health_checked_at TIMESTAMPTZ | query_ms INT | is_dns_primary BOOLEAN | dns_forwarders TEXT[] | **agent_hub_id / dhcp_log_path (per-server DHCP audit-log path for AGENT collection; NULL = agent reads its own local C:WindowsSystem32dhcp. Central collection uses WinRM, not this) TEXT (IDX `idx_ddi_servers_agent_hub`; Phase 4b — NULL = polled centrally, non-NULL = owned by that hub agent; getActiveServers filters `agent_hub_id IS NULL`)** | **(+ auth_mode, ps_username, ps_password [AES-256-GCM encrypted], winrm_port, winrm_https, winrm_test_ok, winrm_tested_at, notes from schema-server-auth.sql)** | **(+ site_id INT from schema-sites.sql, references netvault.sites.id cross-DB, no FK)**

`ddi_agents`  id(PK,SERIAL) | hub_agent_id TEXT UQ NOT NULL (hub's durable agent id "agt_…") | name TEXT | hostname TEXT (reported on each heartbeat; `name` adopts it while still equal to the placeholder hub id, so the UI shows the machine not "agt_…" — 1.25.0) | status TEXT DEFAULT 'online' | version TEXT | last_seen_at TIMESTAMPTZ | created_at TIMESTAMPTZ — Phase 4b. One row per remote data-plane agent connected to the WS ingest server (`api/ws-server.js`, port `DDI_WS_PORT`/3011). Auto-created on first authenticated connect (hub-signed JWT, no stored credential → no secret column). Readonly SELECT granted per-table (non-blanket) to nocvault_readonly + claude_readonly in schema.sql.
...
`app_settings`  key(PK,TEXT) | value TEXT | updated_at TIMESTAMPTZ | seeded with app_name/app_subtitle/company_name/theme/retention_days; `scan_dns_server` is a runtime key (no seed row, set via Settings UI), read by `collector/ipamScanner.js`
...
`smtp_config`  id(PK,SERIAL) | host TEXT NOT NULL | port INT DEFAULT 587 | secure BOOLEAN | username TEXT | **password TEXT — AES-256-GCM encrypted via credStore.js (SECRET column)** | from_email TEXT NOT NULL | from_name TEXT | enabled BOOLEAN | updated_at TIMESTAMPTZ | (2026-07-23: `smtp_config_public` view added — every column except `password` — see Privilege notes)
...
Also in this file: `update_updated_at()` trigger function (used by `ddi_servers`, `ipam_subnets`, `ipam_supernets`, `ipam_addresses`); a `nocvault_readonly`/`claude_readonly` cross-DB grant DO-block (GRANT SELECT ON ALL TABLES + ALTER DEFAULT PRIVILEGES, no-op if role absent), followed by narrowing REVOKE/GRANT blocks for `app_settings`/`api_keys` (views) and `smtp_config` (view) — see Privilege notes. Three views: `app_settings_public`, `api_keys_public`, `smtp_config_public`. **The `ddi_servers` column-level grant is NOT here** — it was relocated to the end of `schema-sites.sql` on 2026-07-25 because its column list references `auth_mode`/`winrm_*`/`site_id`, which don't exist on `ddi_servers` until `schema-server-auth.sql` (3rd file) and `schema-sites.sql` (4th) run; keeping it in `schema.sql` (1st) made it error on every fresh install (`column "site_id" ... does not exist`) and, under `-v ON_ERROR_STOP=1`, abort the whole suite install.
...
- Full audit (2026-07-23): grepped all 4 schema files for `password|secret|_token|api_key|credential` — the four columns above are the complete list. Nothing else was missed.
...
**FIXED 2026-07 (app_settings/api_keys), then CORRECTED + completed 2026-07-23 (ddi_servers/smtp_config):** `nocvault_readonly`/`claude_readonly` no longer get table-level SELECT on `app_settings`, `api_keys`, `ddi_servers`, or `smtp_config` at all.
- `app_settings` → `app_settings_public` view: `app_name`/`app_subtitle`/`company_name`/`theme`/`retention_days`/`scan_dns_server` (the last two added 2026-07-23 — they were genuinely non-secret but had been missed from the original allowlist).
- `api_keys` → `api_keys_public` view: every column except `key_hash`.
- `ddi_servers` → **column-level `GRANT SELECT (...)`** excluding `ps_username`/`ps_password` (no view — this table has no secret/non-secret mix packed into one generic column the way `app_settings` does, so a plain column allowlist is sufficient). **This block lives at the END of `schema-sites.sql` (the 4th/last schema file), NOT in `schema.sql` — see the note below and the 2026-07-25 relocation.** The grant list is all 25 `ddi_servers` columns minus the 2 secrets = 23 columns; live-verified against production `ddi_servers` on 2026-07-25 (`claude_readonly` sees exactly those 23; `ps_username`/`ps_password` are invisible to it, which is the fix working).
- `smtp_config` → `smtp_config_public` view: every column except `password`.
- **The 2026-07 pass's original code comment (and this file's own previous text) claimed `smtp_config`/`ddi_servers.ps_password` were "already safe" because no `*_public` view existed for them — this was WRONG.** Not creating a view does nothing to revoke a table's pre-existing blanket `GRANT SELECT ON ALL TABLES`; both `ddi_servers.ps_password` and `smtp_config.password` were live-verified STILL fully readable by `claude_readonly` on 2026-07-23, the whole time the "FIXED" claim stood. Actually fixed now (the `app_settings`/`api_keys`/`smtp_config` REVOKE/GRANT blocks in `scripts/schema.sql`; the `ddi_servers` one in `scripts/schema-sites.sql`).
- **Cross-file ordering (2026-07-25 fix):** the `ddi_servers` column-grant was originally placed in `schema.sql` alongside the other three, but its column list names `auth_mode`/`winrm_*`/`notes` (added by `schema-server-auth.sql`, 3rd file) and `site_id` (added by `schema-sites.sql`, 4th file). `schema.sql` runs FIRST, so on a fresh install the grant errored (`column "site_id" of relation "ddi_servers" does not exist`) and, under the `-v ON_ERROR_STOP=1` now used everywhere, aborted the entire suite install (pre-`ON_ERROR_STOP` it silently no-op'd, leaving the secrets exposed on fresh installs). It's now at the end of `schema-sites.sql`, the last file applied by both the per-app updater and the suite installer, where all 25 columns exist and it is the final statement touching these roles' `ddi_servers` privileges. The other three tables' narrowing blocks stay in `schema.sql` — those tables and their columns are all created in that same file, so they have no ordering problem.
- A REVOKE+GRANT (view or column-level) block runs after the blanket grant, every time the schema files run, for all four tables (three in `schema.sql`, `ddi_servers` in `schema-sites.sql`) — so none of this can be silently re-opened by a future deploy re-running the blanket grant.
...
**(b) No dedicated readonly DB role/grant script ships in this repo's `scripts/` or `installer/`.** Confirmed: `scripts/` contains only the 4 schema files + `seed-smoke-test.sql`/`clean-smoke-test.sql` (no `schema-grants.sql`); `installer/` contains only `Update-DDIVault.ps1`. There IS a `nocvault_readonly` role grant, but it lives in two places, both OUTSIDE a dedicated grants file:
  1. **`scripts/schema.sql` itself** — a DO-block that, if the `nocvault_readonly` role already exists (created elsewhere), grants it `USAGE` on schema public + `SELECT` on all tables + sets `ALTER DEFAULT PRIVILEGES` so future `ddivault_user`-created tables are auto-covered — followed immediately (2026-07, extended 2026-07-23) by the `app_settings_public`/`api_keys_public`/`smtp_config_public` narrowing blocks above. (The `ddi_servers` column-grant narrowing lives at the end of `schema-sites.sql` instead — see the 2026-07-25 cross-file-ordering note in Privilege notes.) No-op on a standalone install with no Hub role.
  2. **The suite installer** `../netvault/installer/Install-NocVault-Suite.ps1` does the DDIVault-specific provisioning via a shared helper, `GrantNocRoRead "ddivault"` — an unconditional `GRANT CONNECT`/`USAGE`/`SELECT ON ALL TABLES`/`ALTER DEFAULT PRIVILEGES` to `nocvault_readonly` (never `claude_readonly`). **Reordered 2026-07**: this now runs BEFORE the 4 schema files apply, not after — it used to run after, which meant its own blanket grant executed AFTER `schema.sql`'s narrowing fix and silently undid it on every fresh suite install (whichever grant runs last wins in Postgres). Running it before any table exists is safe: `GRANT SELECT ON ALL TABLES` on zero tables is a no-op, and the `ALTER DEFAULT PRIVILEGES` it sets only auto-grants SELECT on tables about to be created, which `schema.sql`'s own later REVOKE still correctly narrows regardless of how the grant first landed. Also note this installer path only ever grants `nocvault_readonly`, never `claude_readonly` — `claude_readonly`'s access on a fresh suite install exists only via whatever manual out-of-band grant provisions it (see below), and `schema.sql`'s narrowing block is a safe no-op (`IF EXISTS` guard) until it does.

  The separate **`claude_readonly`** Postgres user (mentioned in this app's `CLAUDE.md` "Database Access (Read-Only Diagnostics)" section, password out-of-repo in Claude project memory) was previously entirely outside any schema file — confirmed as of 2026-07-23 it now IS covered by the narrowing blocks (`app_settings_public`/`api_keys_public` views + `smtp_config_public` view in `scripts/schema.sql`; `ddi_servers` column-grant in `scripts/schema-sites.sql`), but its INITIAL blanket grant (however it originally got table-level access) is still not something any file in this repo creates — it was set up manually, out-of-band. If `claude_readonly` doesn't exist on a given server, the `IF EXISTS` guard makes the narrowing block a safe no-op there too.
