...
/**
 * agent-identity.js — verify a hub-signed agent JWT on DDIVault's data plane.
 *
 * Phase 4b (identity reconciliation): the NocVault hub (NetVault) issues each
 * agent a durable, HS256-signed JWT with the suite-shared NEXTAUTH_SECRET — the
 * same trust root SSO already uses. DDIVault verifies that signature to accept
 * an agent's WebSocket data tunnel (api/ws-server.js) instead of minting/checking
 * its own api_key.
 *
 * This is a faithful, verbatim port of SpanVault's api/agent-identity.js (itself
 * a port of the hub's netvault/lib/agentIdentity.ts verifyAgentIdentity) — kept
 * ~identical across the suite so all four apps share one trust contract. Reuses
 * the `jsonwebtoken` dep (added to DDIVault's package.json in Phase 4b).
 *
